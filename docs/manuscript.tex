...
Given the sample size they report, the study is underpowered \footnote{\citeA{johansen2007paradoxical} reports a sample size of $n = 16$. If we assume an effect size of $d = 0.46$ observed in \citeA{inkster2022neural} and an $\alpha = 0.05$ with a non-directional alternative hypothesis, the experiment has $power = 0.24$. We used the method provided by the R package pwr \cite{champely2020pwr} to calculate power.}.
...
Interestingly, there was no difference between the fixation time of $C$ and $B$ during $BC$ trials.
...
It is reasonable to suggest that attentional occurs in a standard supervised learning paradigm and it is driven by prediction error.
