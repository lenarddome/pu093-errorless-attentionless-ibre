...
\citeA{johansen2007paradoxical} report a sample size of $n = 16$.
If we use an effect size of $d = 0.46$ observed by \citeA{inkster2022neural} and an $\alpha = 0.05$ with a non-directional alternative hypothesis, the experiment has $power = 0.24$\footnote{We used the method provided by the R package pwr \cite{champely2020pwr} to calculate power.}.
...
Furthermore, \citeA{wills2014attention} observed posterior selection negativity and concurrent frontal positivity for C relative to B, which gave evidence for an error-driven selective attentional learning process.
...
These three studies, \citeA{kruschke2005eye}, \citeA{wills2014attention} and \citeA{inkster2022neural} gave strong evidence in support of the error-driven attentional learning accounts of the IBRE.
...
We recruited 122 undergraduate students who completed the experiment for partial course credit.
...
In addition, we also excluded participants whose mean reaction time fell one standard deviation, $\sigma = 519.96$ ms, below the group mean, $\mu = 1341.57$ ms.


\subsection{Results and Discussion}

After exclusion, 59 participants made it into our analysis.
...
Participants showed a clear common preference for stimuli A, $M = 0.78$, 95\% HDI $[0.71, 0.85]$, $\mathrm{BF}_{10} = 7.72 \times 10^{8}$.
...
Participants also showed a reliable rare preference on ambiguous BC trials, $M = 0.73$, 95\% HDI $[0.66, 0.80]$, $\mathrm{BF}_{10} = 581,370.03$.
This gives us a sufficient amount of evidence to conclude that we have observed the IBRE.
...
Across two experiments, we investigated whether the demonstration of the IBRE requires the "guess-and-correct" components of the standard experimental procedure.
