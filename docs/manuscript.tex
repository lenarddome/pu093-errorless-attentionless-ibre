...
In this experiment, we implemented the IBRE in a way most similar to cued-recall memory tasks.
...
We recruited 65 undergraduate students who completed the exeriment for partial course credit.
...
I have a myriads of things to say here. \\
...
The group-level mean probabilities are shown on Table \ref{tab:results-exp2}.
The resuls are a qualitative and ordinal match to Experiment 1.
Participants showed a clear common preference for stimuli A, $M_{A} = 0.21$, 95\% HDI $[0.11, 0.31]$, $\mathrm{BF}_{10} = 11,312.94$.

\begin{table}[H]
  \begin{center}
    \caption{Group-level mean probabilities for each stimulus
    presented during the test phase in Experiment 2 after exclusion.\\}
...
Therefore we conclude that we reliably detected the IBRE.
